...
description: This post describe the diffusion model formulation in SO(3) space.
tags: AI
citation: true
---
...
\begin{equation}
r^t=e^{\theta_t \mathbf{n}} r^0
\end{equation}
where $$\theta_t$$ is sampled from the IGSO(3) distribution with standard deviation $$\sigma_t$$, and $$\mathbf{n}$$ is a 3D vector sampled from the uniform distribution.
...
\begin{equation}
r^t=e^{\beta_t \mathbf{n}} r^{t-1}
\end{equation}
where $$\beta_t$$ is sampled from the IGSO(3) distribution with standard deviation $$\sqrt{\sigma^2_t - \sigma^2_{t-1}}$$, and $$\mathbf{n}$$ is a 3D vector sampled from the uniform distribution.
